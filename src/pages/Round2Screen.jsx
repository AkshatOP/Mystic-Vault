...
function generateMaze(cols, rows) {
  // Simple Mulberry32 PRNG to guarantee the same maze generation every time
  let seed = 1337;
  function random() {
  ...
  const cells = Array.from({ length: rows }, (_, r) =>
    Array.from({ length: cols }, (_, c) => ({ r, c, walls: { N: true, S: true, E: true, W: true }, visited: false, explored: false, isCheckpointP1: false, isCheckpointP2: false }))
  );
  ...
  while (stack.length) {
    ...
    const ns = unvisitedNeighbors(cur.r, cur.c);
    if (ns.length) {
...
export default function Round2Screen({ audio }) {
  ...
  function loop() {
    ...
    if (DEV_DISABLE_DARKNESS) {
      visRef.current = 1000; // Uncapped vision
    }
  ...
  function drawMaze() {
    ...
    cells.forEach((row, r) => row.forEach((cell, c) => {
      ...
      const rawOp = (roundDone.current || DEV_DISABLE_DARKNESS) ? 1 : Math.max(0, 1 - dist / (VIS + 1.8));
    ...
    // ── Vision shrink progress bar ──────────────────────────
    {
      ...
      if (elapsed > VISION_GRACE_S && !DEV_DISABLE_DARKNESS) {
  ...
  useEffect(() => {
    ...
    // Rules: Left -> left, Right -> right, Up -> up, Down -> down
    const p1Map = {
      'ArrowLeft': [0, -1],  // Left Arrow = left
      'ArrowRight': [0, 1],  // Right Arrow = right
      'ArrowUp': [-1, 0],    // Up Arrow = up
      'ArrowDown': [1, 0]    // Down Arrow = down
    };

    // P2 (Green, Left): Uses WASD
    // Rules: W -> up, S -> down, A -> left, D -> right
    const p2Map = {
      'w': [-1, 0], 'W': [-1, 0],  // W = up
      's': [1, 0], 'S': [1, 0],    // S = down
      'a': [0, -1], 'A': [0, -1],  // A = left
